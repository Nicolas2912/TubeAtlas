...
class KnowledgeGraphBuilder:
    def __init__(self, model_name="Babelscape/rebel-large", use_gpu=True):
        ...
        # Set device
        self.device = "cuda" if use_gpu and hasattr(torch, 'cuda') and torch.cuda.is_available() else "cpu"
    ...
    def process_file(self, file_path, chunk_size=512, overlap=50):
        ...
        try:
            ...
            sentences = [s.strip() for s in self.sentence_splitter.split(file_content) if s.strip()]
    ...
    def collect_and_process_all_files(self, directory_path, file_extensions=["txt", "md", "csv"], 
                                    chunk_size=512, overlap=50, n_processes=4):
    ...
    def process_chunks_for_triplets(self, chunks, batch_size=64, max_workers=4):
        ...
        print(f"Using device: {self.device}, batch size: {batch_size}, workers: {max_workers}")
        ...
        if self.device == "cuda":
            ...
            dummy_input = self.tokenizer("Warming up the GPU", return_tensors="pt").to(self.device)
            self.model.eval()
            with torch.no_grad():
        ...
        def tokenization_worker():
            ...
            while True:
                try:
                    ...
                    # Tokenize this batch
                    result = self.prepare_batch_inputs(batch_idx, batches, self.tokenizer)
        ...
        try:
            while batches_processed < total_batches:
                ...
                try:
                    ...
                    if current_time - last_update > 5.0:  # Every 5 seconds
                        ...
                        eta = (total_batches - batches_processed) / rate if rate > 0 else float('inf')
            ...
            print(f"Extracted {len(all_triplets)} unique triplets from {sum(len(seen_triplets) for _ in [1])} total")
    ...
    def analyze_knowledge_graph(self, G):
        ...
        top_entities = sorted(degree_centrality.items(), key=lambda x: x[1], reverse=True)[:20]
    ...
    def save_knowledge_graph(self, G, output_dir="knowledge_graph_output"):
        ...
        edge_df.to_csv(os.path.join(output_dir, "knowledge_graph_edges.csv"), index=False)
        
        nodes_df = pd.DataFrame({"node": list(G.nodes())})
        nodes_df.to_csv(os.path.join(output_dir, "knowledge_graph_nodes.csv"), index=False)
    ...
    def visualize_knowledge_graph(self, G, output_dir="knowledge_graph_output", max_nodes=100):
        ...
        nx.draw(G_sub, pos, with_labels=True, node_color=node_colors,
                node_size=1500, arrowsize=20, font_size=12, cmap=plt.cm.tab20)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, "knowledge_graph_visualization.png"), dpi=300)
        
        return os.path.join(output_dir, "knowledge_graph_visualization.png")
    
    def create_knowledge_graph_from_directory(self, directory_path, output_dir="knowledge_graph_output",
                                             file_extensions=["txt", "md", "csv"], n_processes=4):
