...
def _fetch_transcript_data(db_name: str) -> list:
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT title, transcript_text, openai_tokens FROM transcripts WHERE transcript_text IS NOT NULL AND transcript_text != ''")
...
def _prepare_input_string(segments: list) -> str:
    ...
    formatted_transcripts = [f"=== Title: {seg['title']} ===\nTranscript: {seg['text']}" for seg in segments]
...
def _get_insights_direct(transcript_segments: list) -> str:
    ...
    logger.info(f"Processing {len(transcript_segments)} transcripts directly (single API call).")
...
async def _get_insights_sliding_window(transcript_segments: list) -> str:
    ...
    logger.info(f"Processing {len(transcript_segments)} transcripts using sliding window.")
    ...
    while segment_idx < len(transcript_segments):
        ...
        if current_window_tokens + segment["tokens"] > WINDOW_SIZE and current_window_segments:
        ...
        if not current_window_segments or current_window_tokens + segment["tokens"] <= WINDOW_SIZE:
            current_window_segments.append(segment)
            current_window_tokens += segment["tokens"]
            segment_idx += 1
        elif not current_window_segments and segment["tokens"] > WINDOW_SIZE: # Single segment too large
            logger.warning(f"Segment '{segment['title']}' ({segment['tokens']} tokens) is larger than window size ({WINDOW_SIZE} tokens) and will be processed alone.")
    ...
    async def process_window(window_segments: list, window_num: int) -> str:
        ...
        logger.info(f"Processing window {window_num}/{len(windows_data)} with {len(window_segments)} transcripts, {sum(s['tokens'] for s in window_segments)} tokens.")
    ...
    tasks = [process_window(window_segments, i+1) for i, window_segments in enumerate(windows_data)]
    ...
    if len(partial_insights_list) == 1:
        ...
        synthesis_prompt = SYNTHESIS_PROMPT_TEMPLATE.format(partial_summaries_text=partial_summaries_text)
        try:
            # Assuming _call_openai_api is updated to be async
            ...
            return "Failed to synthesize final insights. Partial summaries:\n" + partial_summaries_text
...
def _get_insights_batch_api(transcript_segments: list) -> str:
    ...
    logger.info(f"Total tokens ({total_tokens}) exceed threshold for sliding window. Using Batch API.")
        ...
        task = {
            ...
            "body": {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": MAIN_PROMPT_TEMPLATE.format(input_text=f"Title: {segment['title']}\n\nTranscript: {segment['text']}")
                    },
                    {
                        "role": "user",
                        "content": ""
    ...
    try:
        ...
        for result in sorted(results, key=lambda x: int(x['custom_id'].split('-')[1])):
            if 'error' in result:
                logger.error(f"Error in segment {result['custom_id']}: {result['error']}")
                continue
        ...
        synthesis_prompt = "Synthesize these partial insights into a coherent summary:\n\n" + "\n\n".join(insights)
...
def get_batch_job_results(batch_id: str) -> dict:
    ...
    try:
        ...
        if batch_job.status == "completed":
            if batch_job.output_file_id:
                logger.info(f"Fetching content from output file: {batch_job.output_file_id}")
                output_file_content_response = client.files.content(batch_job.output_file_id)
                output_content_str = output_file_content_response.content.decode('utf-8')
                ...
                for line_str in output_content_str.strip().splitlines():
                    ...
                    try:
                        ...
                        if top_level_error_obj:
                            ...
                            error_detail = top_level_error_obj.get('body', {}).get('error', top_level_error_obj)
                            results_payload["processed_output"].append({
                                "custom_id": custom_id,
                                "error": error_detail
                            })
                            logger.warning(f"Top-level error for batch segment {custom_id}: {error_detail.get('message', str(error_detail))}")
                            continue
                        ...
                        if not response_part:
                            results_payload["processed_output"].append({
                                "custom_id": custom_id,
                                "error": {"message": "Missing 'response' field in batch output line."},
                                "raw_segment_response": response_json
                            })
                            logger.warning(f"Missing 'response' field for batch segment {custom_id}. Raw: {response_json}")
                            continue
                        ...
                        if api_error_in_body:
                            results_payload["processed_output"].append({
                                "custom_id": custom_id,
                                "error": api_error_in_body
                            })
                            logger.warning(f"API error in response body for batch segment {custom_id}: {api_error_in_body.get('message')}")
                            continue
                        ...
                        if choices and isinstance(choices, list) and len(choices) > 0:
                            ...
                            if message and isinstance(message, dict):
                                ...
                                if content is not None: # Content can be an empty string which is valid
                                    results_payload["processed_output"].append({
                                        "custom_id": custom_id,
                                        "content": content
                                    })
                                    ...
                                    logger.warning(f"Content is null for batch segment {custom_id}. Raw: {response_json}")
                                ...
                                logger.warning(f"Unexpected message structure for batch segment {custom_id}. Raw: {response_json}")
                             ...
                             logger.warning(f"Unexpected choices structure for batch segment {custom_id}. Raw: {response_json}")
                        parsed_lines +=1
                    except json.JSONDecodeError as e:
                        logger.error(f"Failed to parse JSON line from output file {batch_job.output_file_id}: {e}. Line: '{line_str}'")
                        results_payload["processed_output"].append({
                            "custom_id": "unknown_json_error",
                            "error": {"message": f"JSONDecodeError: {e}", "line_content": line_str}
                        })
                    except Exception as e: # Catch any other unexpected errors during line processing
                        logger.error(f"Unexpected error processing line from output file {batch_job.output_file_id}: {e}. Line: '{line_str}'", exc_info=True)
                        results_payload["processed_output"].append({
                            "custom_id": "unknown_processing_error",
                            "error": {"message": f"Unexpected error: {e}", "line_content": line_str}
                        })
                logger.info(f"Successfully processed {parsed_lines} lines from output file {batch_job.output_file_id}.")
            else:
                results_payload["retrieval_message"] = "Batch job completed, but no output file ID found."
            ...
            if batch_job.errors and batch_job.errors.data:
                for error_obj in batch_job.errors.data:
                 ...
                 results_payload["errors_summary"].append({"message": "Batch job failed, but no specific error details found in batch_job.errors.data."})

            if batch_job.error_file_id:
                logger.info(f"Fetching content from error file: {batch_job.error_file_id}")
                try:
                    error_file_content_response = client.files.content(batch_job.error_file_id)
                    results_payload["error_file_raw_content"] = error_file_content_response.content.decode('utf-8')
                except Exception as e:
                    logger.error(f"Failed to retrieve or decode error file {batch_job.error_file_id}: {e}", exc_info=True)
                    # Add to summary as this is an error in retrieving the error file itself
                    results_payload["errors_summary"].append({"message": f"Failed to retrieve error file content for {batch_job.error_file_id}: {e}"})
            else:
                logger.info(f"Batch job {batch_id} failed, no separate error file ID provided.")
                if not results_payload["errors_summary"] or \
                   (len(results_payload["errors_summary"]) == 1 and results_payload["errors_summary"][0]["message"].startswith("Batch job failed, but no specific error details")):
                     results_payload["errors_summary"].append({"message": "No error file ID and no further details in batch_job.errors.data."})
        else: 
            results_payload["retrieval_message"] = f"Batch job {batch_id} is currently {batch_job.status}."
            logger.info(results_payload["retrieval_message"])

    except Exception as e:
        logger.error(f"Failed to retrieve or process batch job {batch_id}: {e}", exc_info=True)
        results_payload["status"] = results_payload["status"] or "error_during_retrieval" # Preserve status if already set by API
        if results_payload["status"] is None: results_payload["status"] = "error_during_retrieval"
        
        error_message = f"An error occurred while fetching/processing batch job: {str(e)}"
        results_payload["retrieval_message"] = error_message
        # Ensure this top-level error is also in errors_summary
        if not any(item.get("message") == error_message for item in results_payload["errors_summary"]):
            results_payload["errors_summary"].append({"message": error_message, "code": "retrieval_error"})


    # Construct final, cleaner dictionary
    final_results = {"batch_id": results_payload["batch_id"], "status": results_payload["status"]}
    ...
    if results_payload["status"] == "completed":
        ...
        if results_payload["error_file_raw_content"]: # Only include if it has content
            final_results["error_file_raw_content"] = results_payload["error_file_raw_content"]
    
    # Include retrieval message if relevant (e.g., for non-terminal states or overarching errors)
    if results_payload["retrieval_message"] and results_payload["status"] not in ["completed", "failed"]:
        final_results["retrieval_message"] = results_payload["retrieval_message"]
    elif results_payload["status"] == "error_during_retrieval" and results_payload["retrieval_message"]:
         final_results["retrieval_message"] = results_payload["retrieval_message"]
...
def get_channel_insights(db_name: str) -> str:
    ...
    if total_tokens < DIRECT_API_THRESHOLD:
        ...
        logger.info("Note: Sliding window is currently synchronous. Consider async for performance on multiple windows.")
