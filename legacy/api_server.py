...
async def get_datasets():
    ...
    return {"datasets": available_datasets, "count": len(available_datasets)}
...
async def get_knowledge_graph(dataset: str):
    ...
    try:
        ...
        response_data = {
            **kg_data,
            "metadata": {
                ...
                "triple_count": (
                    len(kg_data.get("triples", [])) if "triples" in kg_data else 0
                ),
            },
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Error parsing knowledge graph file: {str(e)}"
        )
    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Error loading knowledge graph: {str(e)}"
        )
