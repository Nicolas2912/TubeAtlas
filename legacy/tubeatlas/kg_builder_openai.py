...
class HybridKGBuilder:
    ...
    def preprocess_transcript(self, transcript: str, max_chars: int = 2000) -> List[str]:
        ...
        logger.info(f"Created {len(chunks)} chunks from {len(selected_sentences)} sentences (max 2000 chars)")
        return chunks
    ...
    def extract_classical_entities(self, text: str) -> Dict[str, List[str]]:
        ...
        for chunk in doc.noun_chunks:
            if len(chunk.text.strip()) > 2 and chunk.text.strip() not in [e['text'] for ents in entities.values() for e in ents]:
        ...
        logger.info(f"Classical extraction found {entity_count} entities: {dict([(k, len(v)) for k, v in entities.items()])}")
    ...
    def extract_classical_relations(self, text: str) -> List[Tuple[str, str, str]]:
        ...
        for sent in doc.sents:
            ...
            for token in sent:
                if token.pos_ == "ADP":  # Preposition
                    if token.head.pos_ in ["NOUN", "PROPN"] and token.nbor(1).pos_ in ["NOUN", "PROPN"]:
        ...
        relations = [(s, r, o) for s, r, o in relations if len(s.strip()) > 1 and len(o.strip()) > 1 and s.strip() != o.strip()]
    ...
    def enhance_with_llm(self, text: str, classical_entities: Dict) -> Dict:
        ...
        entities_str = json.dumps({k: [e['text'] for e in v] for k, v in classical_entities.items()}, indent=2)
        ...
        for attempt in range(max_retries):
            try:
                ...
                required_fields = ["validated_entities", "new_entities", "relationships", "corrections"]
                if not all(field in result for field in required_fields):
                    raise ValueError(f"Missing required fields in response. Got: {list(result.keys())}")
                
                # Log LLM results
                llm_entity_count = sum(len(ents) for ents in result.get("new_entities", {}).values())
                llm_relation_count = len(result.get("relationships", []))
                logger.info(f"LLM enhancement found {llm_entity_count} new entities and {llm_relation_count} relationships")
                ...
                logger.error(f"LLM enhancement attempt {attempt + 1}/{max_retries} failed: {str(e)}")
                if attempt < max_retries - 1:
                    logger.info(f"Retrying in {retry_delay} seconds...")
                    time.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                else:
                    logger.error("All retry attempts failed, falling back to classical extraction only")
                    return {
                        "validated_entities": classical_entities,
                        "new_entities": {},
                        "relationships": [],
    ...
    def assess_entity_quality(self, entity: str, context: str = None) -> Tuple[bool, float]:
        ...
        if self.entity_metrics['min_length'] <= len(entity) <= self.entity_metrics['max_length']:
            factors.append(1.0)
        else:
            factors.append(0.0)
            
        word_count = len([token for token in doc if not token.is_punct])
        if self.entity_metrics['min_word_count'] <= word_count <= self.entity_metrics['max_word_count']:
            factors.append(1.0)
        else:
            factors.append(0.0)
        ...
        is_temporal = any(token.ent_type_ == "DATE" or token.ent_type_ == "TIME" for token in doc)
        ...
        freq_score = min(1.0, self.entity_frequencies[entity.lower()] / 10)  # Normalize frequency
    ...
    def assess_relation_quality(self, subject: str, relation: str, object: str, context: str = None) -> Tuple[bool, float]:
        ...
        # 1. Length checks
        if self.relation_metrics['min_length'] <= len(relation) <= self.relation_metrics['max_length']:
            factors.append(1.0)
        else:
            factors.append(0.0)
        ...
        is_generic = any(token.lemma_ in ["be", "have", "do", "make", "get"] for token in rel_doc)
        if is_generic:
            factors.append(0.3)
        else:
            factors.append(1.0)
        ...
        freq_score = min(1.0, self.relation_frequencies[rel_key] / 5)  # Normalize frequency
        ...
        if subject.lower() != object.lower():  # Avoid self-relations
            ...
            if np.any(subj_vec) and np.any(obj_vec):  # Check if vectors are not zero
                similarity = np.dot(subj_vec, obj_vec) / (np.linalg.norm(subj_vec) * np.linalg.norm(obj_vec))
        ...
        if context:
            ...
            if all(term.lower() in context_text for term in [subject, relation, object]):
                factors.append(1.0)
            else:
                factors.append(0.5)
    ...
    def clean_relation(self, relation: str, subject: str = None, object: str = None, context: str = None) -> Optional[str]:
        ...
        if subject and object:
            is_valid, quality_score = self.assess_relation_quality(subject, relation, object, context)
            if not is_valid:
                return None
    ...
    def deduplicate_relations(self, relations: List[Tuple[str, str, str]], context: str = None) -> List[Tuple[str, str, str]]:
        ...
        for subj, rel, obj in relations:
            ...
            for existing in seen:
                ...
                if (self.are_entities_similar(clean_subj, ex_subj) and 
                    self.are_entities_similar(clean_obj, ex_obj) and 
                    self.are_relations_similar(clean_rel, ex_rel)):
                    is_similar = True
                    break
    ...
    def merge_extractions(self, classical_entities: Dict, classical_relations: List[Tuple], llm_enhanced: Dict) -> Dict:
        ...
        for rel in llm_enhanced.get("relationships", []):
            if len(rel) >= 4 and rel[3] > self.relation_metrics['min_confidence']:
                ...
                cleaned_rel = self.clean_relation(rel_type, cleaned_subj, cleaned_obj, context)
        ...
        logger.info(f"Merged results: {sum(len(ents) for ents in merged['entities'].values())} entities, {len(merged['relations'])} relations")
    ...
    def build_graph_from_chunk(self, chunk: str) -> None:
        ...
        merged = self.merge_extractions(classical_entities, classical_relations, llm_results)
    ...
    def process_transcript(self, transcript: str, max_chars: int = 2000) -> nx.MultiDiGraph:
    ...
    def post_process_graph(self) -> None:
        ...
        nodes_to_remove = low_degree_nodes[:len(low_degree_nodes)//3]  # Remove only 1/3 of isolated nodes
        self.graph.remove_nodes_from(nodes_to_remove)
        
        logger.info(f"Post-processing: removed {len(nodes_to_remove)} isolated nodes ({initial_nodes} -> {self.graph.number_of_nodes()})")
    ...
    def print_graph_details(self):
        ...
        for source, target, data in self.graph.edges(data=True):
            logger.info(f"  - '{source}' --[{data.get('relation', 'related_to')}]--> '{target}'")
